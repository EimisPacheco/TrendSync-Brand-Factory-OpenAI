...
import {
  TrendingUp, MapPin, Calendar, Users, Loader, RefreshCw,
  Palette, Shirt, Layers, Sparkles, Info, Star, Globe,
  Footprints, Watch,
} from "lucide-react";
import { fetchTrends, fetchCelebrities } from "@/lib/api";
import type { TrendInsights } from "@/types/database";
...
function getCelebrityEmoji(name: string, profession: string): string {
  ...
  if (profLower.includes("sing") || profLower.includes("music")) return "🎤";
  if (profLower.includes("act")) return "🎬";
  if (profLower.includes("athlete") || profLower.includes("sport")) return "⚡";
  if (profLower.includes("model")) return "👗";
  if (profLower.includes("influencer")) return "✨";
  if (profLower.includes("fashion")) return "👠";
...
function TrendCard({ item, icon }: { item: TrendItemData; icon: React.ReactNode }) {
  return (
      ...
      <div className="space-y-3">
        <div className="flex items-start justify-between">
          ...
          <div className="flex flex-col items-end">
            <span className="text-[10px] uppercase tracking-wider text-pastel-muted mb-0.5">
              Trend Confidence
            </span>
            <span className="text-sm font-bold text-pastel-accent">
              {item.confidence}%
            </span>
          </div>
        </div>

        <p className="text-sm text-pastel-text-light leading-relaxed mb-3">
          {item.description}
        </p>
        ...
        <div className="flex items-center gap-3">
          ...
          <span
            className={`text-xs font-semibold whitespace-nowrap px-3 py-1.5 rounded-full transition-all ${
              item.confidence >= 85
...
export default function TrendsPage() {
  ...
  const handleFetchRegional = async () => {
    setLoading(true);
    setError(null);
    setInsights(null);
    try {
      ...
      setInsights(res.insights as unknown as TrendInsights);
    } catch (err) {
  ...
  const handleFetchCelebrity = async () => {
    ...
    setInsights(null);
    try {
      // Fetch both the celebrity trend insights and the celebrity list
      const [trendRes, celebRes] = await Promise.all([
        fetchTrends({ demographic, trend_source: "celebrity" }),
        fetchCelebrities(demographic),
      ]);
      setInsights(trendRes.insights as unknown as TrendInsights);
      setCelebList((celebRes.celebrities || []) as unknown as CelebrityBasic[]);
    } catch (err) {
      ...
      {activeTab === "regional" ? (
        <>
          <div className="neumorphic-card p-7">
            <h3 className="text-2xl font-bold text-pastel-navy mb-6">
              Configure Regional Analysis
            </h3>
            ...
            <div className="space-y-6">
              ...
              <div className="neumorphic-card p-8">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <h3 className="text-2xl font-bold text-pastel-navy">
                    Trend Summary
                  </h3>
                </div>
                <p className="text-pastel-text leading-relaxed text-lg">
                  {insights.summary}
                </p>
              </div>
              ...
              <div className="space-y-8">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h3 className="text-xl font-bold text-pastel-navy">
                      Apparel Trends
                    </h3>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {insights.colors.slice(0, 2).map((item) => (
                      <TrendCard
                        key={item.name + "-apparel"}
                        item={{ ...item, name: `${item.name} (Apparel)` }}
                        icon={<Palette className="text-purple-500" size={20} />}
                      />
                    ))}
                    {insights.silhouettes.slice(0, 2).map((item) => (
                      <TrendCard
                        key={item.name + "-sil"}
                        item={item}
                        icon={<Shirt className="text-pastel-accent" size={20} />}
                      />
                    ))}
                    {insights.materials.slice(0, 2).map((item) => (
                      <TrendCard
                        key={item.name + "-mat-apparel"}
                        item={{ ...item, name: `${item.name} (Fabric)` }}
                        icon={<Layers className="text-pastel-teal" size={20} />}
                      />
                    ))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {insights.colors.slice(2, 3).map((item) => (
                      <TrendCard
                        key={item.name + "-footwear-color"}
                        item={{ ...item, name: `${item.name} (Footwear)` }}
                        icon={<Palette className="text-purple-500" size={20} />}
                      />
                    ))}
                    {insights.silhouettes.slice(2, 3).map((item) => (
                      <TrendCard
                        key={item.name + "-shoe"}
                        item={{ ...item, name: `${item.name} (Shoe Style)` }}
                        icon={<Footprints className="text-blue-500" size={20} />}
                      />
                    ))}
                    {insights.materials.slice(2, 3).map((item) => (
                      <TrendCard
                        key={item.name + "-mat-footwear"}
                        item={{ ...item, name: `${item.name} (Footwear)` }}
                        icon={<Layers className="text-pastel-teal" size={20} />}
                      />
                    ))}
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {insights.colors.slice(3, 4).map((item) => (
                      <TrendCard
                        key={item.name + "-acc-color"}
                        item={{ ...item, name: `${item.name} (Accessories)` }}
                        icon={<Palette className="text-purple-500" size={20} />}
                      />
                    ))}
                    {insights.themes.map((item) => (
                      <TrendCard
                        key={item.name + "-theme"}
                        item={{ ...item, name: `${item.name} Theme` }}
                        icon={<Sparkles className="text-amber-500" size={20} />}
                      />
                    ))}
                    {insights.materials.slice(3).map((item) => (
                      <TrendCard
                        key={item.name + "-mat-acc"}
                        item={{ ...item, name: `${item.name} (Accessories)` }}
                        icon={<Layers className="text-pastel-teal" size={20} />}
                      />
                    ))}
        ...
        <>
          <div className="neumorphic-card p-7">
            <h3 className="text-2xl font-bold text-pastel-navy mb-4">
              Celebrity Fashion Analysis
            </h3>
            <p className="text-pastel-text mb-6">
              Discover fashion trends from the top 10 most influential celebrities
              of the past 5 years. The analysis includes actors, musicians,
              athletes, and fashion icons who shape global fashion trends.
            </p>

            <button
              onClick={handleFetchCelebrity}
              disabled={loading}
              className="w-full py-4 px-6 btn-navy text-lg flex items-center justify-center gap-2 disabled:opacity-50"
            >
              {loading ? (
          ...
          {(insights || celebList.length > 0) && (
            <div className="space-y-6">
                      ...
                      <div className="space-y-4">
                        <div className="flex items-start gap-4">
                          ...
                          <div className="w-20 h-20 rounded-full overflow-hidden neumorphic-sm flex-shrink-0">
                            <div
                              className={`w-full h-full flex items-center justify-center text-4xl bg-gradient-to-br ${
                                GRADIENT_CLASSES[index % GRADIENT_CLASSES.length]
                              }`}
                            >
                              {getCelebrityEmoji(celeb.name, celeb.profession)}
                            </div>
                          </div>
                          ...
                          <div className="flex-1">
                            <div className="flex items-start justify-between">
                              <div>
                                <h4 className="font-bold text-lg text-pastel-navy">
                                  {celeb.name}
                                </h4>
                        ...
                        <p className="text-sm text-pastel-text leading-relaxed">
                          {celeb.signature_style}
                        </p>
                      </div>
                    </div>
                  ))}
              ...
              {insights && (
                <div className="neumorphic-card p-8">
                  <h3 className="text-xl font-bold text-pastel-navy mb-6">
                    Common Celebrity Trends
                  </h3>
                      ...
                      <div className="space-y-2">
                        {insights.colors.map((color, idx) => (
                          <div key={idx} className="flex items-center gap-2">
                            ...
                            />
                            <span className="text-sm text-pastel-text">
                              {color.name}
                            </span>
                            <div className="ml-auto flex flex-col items-end">
                              <span className="text-[9px] uppercase tracking-wider text-pastel-muted">
                                Confidence
                              </span>
                              <span className="text-xs font-semibold text-pastel-accent">
                                {color.confidence}%
                              </span>
                            </div>
                          </div>
                    ...
                    <div>
                      ...
                      <ul className="space-y-2">
                        {insights.silhouettes.map((style, idx) => (
                          <li key={idx} className="text-sm text-pastel-text">
                            • {style.name}
                          </li>
                        ))}
                      </ul>
                  ...
                  {insights.themes && insights.themes.length > 0 && (
                    <div className="mt-6 p-4 neumorphic-inset rounded-xl">
                      <p className="text-sm text-pastel-text-light leading-relaxed">
                        <Info
                          size={16}
                          className="inline mr-2 text-pastel-accent"
                        />
                        Key themes:{" "}
                        {insights.themes.map((t) => t.name).join(", ")}
                      </p>
                    </div>
                  )}
                </div>
