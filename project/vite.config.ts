  ...
  return {
    ...
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        // Handle /api/bria/*, /api/perplexity/*, and /api/resend/* requests
        const isBria = req.url?.startsWith('/api/bria');
        const isPerplexity = req.url?.startsWith('/api/perplexity');
        const isResend = req.url?.startsWith('/api/resend');

        if (!isBria && !isPerplexity && !isResend) {
          return next();
        }

        const apiName = isBria ? 'Bria' : (isPerplexity ? 'Perplexity' : 'Resend');
        ...
        try {
          ...
          if (isBria) {
            ...
            // Build Perplexity API URL
            const perplexityPath = req.url.replace('/api/perplexity', '');
            targetUrl = `https://api.perplexity.ai${perplexityPath}`;

            // Get Authorization token from request headers
            const authToken = (req.headers['authorization'] as string);
            console.log('🔑 Auth Token found:', authToken ? 'Bearer ***' : '❌ MISSING!');
...
export default defineConfig({
  plugins: [
    react(),
    apiProxyPlugin(), // Custom proxy for Bria and Perplexity APIs
  ],
  optimizeDeps: {
