...
export class GenerationLogger {
  ...
  public async logPerplexityRequest(prompt: string) {
    await this.logSection('Step 1: Perplexity API - Trend Analysis');
    await this.log('Perplexity Request', { prompt }, 'info');
  }

  public async logPerplexityResponse(response: any, parsed: any) {
    await this.log('Perplexity Raw Response', response, 'info');
    await this.log('Perplexity Parsed Data', parsed, 'success');
  }
