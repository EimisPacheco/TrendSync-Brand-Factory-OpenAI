...
import { GeminiTrendService } from './gemini-trends';
...
export class CollectionGeneratorV2 {
  ...
  private async getTrendInsights(config: CollectionConfig, logger?: GenerationLogger): Promise<TrendInsights> {
    if (!this.geminiApiKey) {
      throw new CollectionGeneratorError(
        'Gemini API key not configured. Cannot fetch fashion trends.',
        'trends'
      );
    }
    ...
    try {
      ...
      console.log('Cache MISS - Fetching trends from Gemini + Google Search');
      ...
      const trendService = new GeminiTrendService(this.geminiApiKey);
      const insights = await trendService.fetchTrends({
        season: config.season,
        region: config.region,
        demographic: config.demographic,
        trendSource: config.trendSource,
      });

      if (logger) {
        await logger.log('Gemini Trend Response', {
      ...
      await setCached(cacheKey, insights, CACHE_TTL.PERPLEXITY_TRENDS);
      ...
      throw new CollectionGeneratorError(
        `Failed to fetch trends from Gemini: ${error instanceof Error ? error.message : 'Unknown error'}`,
        'trends'
      );
    }
  }
