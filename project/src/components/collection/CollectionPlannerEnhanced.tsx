...
import { MapPin, Calendar, Users, Shirt, Footprints, Watch, Sparkles, TrendingUp, Loader, Globe, Star, RefreshCw } from 'lucide-react';
    ...
    if (activeTab === 'celebrity' && config.demographic && (celebrityColors.length === 0 || lastCelebrityDemographic !== config.demographic)) {
      const fetchCelebrityColors = async () => {
        try {
          ...
          if (!geminiApiKey) {
            setCelebrityError('Gemini API key is not configured. Please add VITE_GEMINI_API_KEY to your .env file');
            return;
          }

          const { GeminiTrendService } = await import('../../services/gemini-trends');
          const trendService = new GeminiTrendService(geminiApiKey);
          const insights = await trendService.fetchTrends({
            demographic: config.demographic,
            trendSource: 'celebrity',
          });

          if (insights.colors && insights.colors.length > 0) {
            const colors = insights.colors
...
The TodoWrite tool hasn't been used recently. If you're working on tasks that would benefit from tracking progress, consider using the TodoWrite tool to track progress. Also consider cleaning up the todo list if has become stale and no longer matches what you are working on. Only use it if it's relevant to the current work. This is just a gentle reminder - ignore if not applicable.
  ...
  const handleSubmit = () => {
    ...
    const collectionName = activeTab === 'celebrity'
      ? config.name || `Celebrity-Inspired ${finalDemo} Collection`
      : config.name || `${finalSeason} ${finalRegion.split(',')[0]} Collection`;
        ...
        <>
          ...
          <div className="neumorphic-card p-6">
            ...
            <input
              ...
              className="w-full input-neumorphic px-4 py-3 text-pastel-navy mb-4"
            />
        ...
        <>
          ...
          <div className="neumorphic-card p-6">
            ...
            <input
              ...
              value={config.name}
              onChange={(e) => setConfig({ ...config, name: e.target.value })}
              placeholder="Celebrity collection name (optional)"
              className="w-full input-neumorphic px-4 py-3 text-pastel-navy mb-4"
            />
