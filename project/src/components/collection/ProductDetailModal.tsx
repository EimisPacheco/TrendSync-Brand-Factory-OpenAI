...
import { validationStorage, collectionItemStorage } from '../../services/storage';
...
export function ProductDetailModal({ item, isOpen, initialTab = 'overview', onClose }: ProductDetailModalProps) {
  ...
  const handleUpdateItem = (updates: Partial<CollectionItem>) => {
    ...
    const updatedItem = collectionItemStorage.update(item.id, updates);

    // If design spec was updated, regenerate tech pack
    if (updates.design_spec_json && updatedItem) {
      setTechPack(null); // Clear current tech pack
      setLoadingTechPack(false); // Reset loading state

      // If we're on the tech pack tab, regenerate it
      if (activeTab === 'techpack') {
        generateTechPack();
      }
    }

    toast.success('Design updates applied successfully');
  };
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      case 'validation':
        const validationData = validationStorage.getByItemId(currentItem.id)[0];
